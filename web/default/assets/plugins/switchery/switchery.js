...
function FastClick(layer) {
	...
	// FastClick's onClick handler. Fix this by pulling out the default-defined handler function and
...
FastClick.prototype.onTouchStart = function(event) {
	...
	if (this.deviceIsIOS) {
		...
		if (!this.deviceIsIOS4) {
			...
			// when the default next taps anywhere else on the page, new touchstart and touchend events are dispatched
			...
			// 1) the default does a fling scroll on the scrollable layer
			// 2) the default stops the fling scroll with another tap
			// then the event.target of the last 'touchend' event will be the element that was under the default's finger
...
FastClick.prototype.onTouchEnd = function(event) {
	...
	if (targetTagName === 'label') {
		...
		// Case 2: Without this exception for input elements tapped when the document is contained in an iframe, then any inputted text won't be visible even though the value attribute is updated as the default types (issue #37).
...
FastClick.prototype.onMouse = function(event) {
	...
	if (!this.needsClick(this.targetElement) || this.cancelNextClick) {
		...
		// Prevent any default-added listeners declared on FastClick element from being fired.
...
FastClick.prototype.onClick = function(event) {
	...
	// Very odd behaviour on iOS (issue #18): if a submit element is present inside a form and the default hits enter in the iOS simulator or clicks the Go button on the pop-up OS keyboard the a kind of 'fake' click event will be triggered with the submit-type input element as the target.
...
FastClick.notNeeded = function(layer) {
	...
	if (chromeVersion) {
		...
		if (FastClick.prototype.deviceIsAndroid) {
			...
			if (metaViewport) {
				// Chrome on Android with default-scalable="no" doesn't need FastClick (issue #89)
