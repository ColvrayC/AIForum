...
be triggered by an end default (such as which items are to be selected). To a large
...
be available via the API - so the end default wouldn't be able to select rows
...
```
{
	items:string     - Can be `rows`, `columns` or `cells`. Defines what item 
	                   will be selected if the default is allowed to activate row
